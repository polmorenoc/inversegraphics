...
azimuths = [180, 90, 135, 270, 180, 45]
elevations = [45, 75, 45, 45, 5]
azchanges = [10, 15, 5, 0, 15]
elchanges = [10, 0, 5,15, 15]
camDistance = 0.4
light_ints = [0.4, 0.4, 0.4, 0.4]
light_changes = [-0.1, 0.1, 0.2, 0.2]
...
uvmod = [uv[0]]
vnmod = [vn[0]]
haveTexturesmod_list = [haveTextures_list[0]]
texturesmod_list = [textures_list[0]]
...
chAzScaled = ch.Ch([radians(azimuths[experiment]/azScale)])
chElScaled = ch.Ch([radians(elevations[experiment]/elScale)])
chDistScaled = ch.Ch([camDistance/distScale])
chComponentScaled = ch.Ch([ 0.71177498,0,0,0,0,0,0,0,0])
...
light_pos=ch.array(center)
light_pos[1] =  light_pos.r[1] + 1
light_intensity = ch.Ch([light_ints[experiment]])

# A_list = [SphericalHarmonics(vn=vnch[mesh], components=chComponent, light_color=light_color) for mesh in rangeMeshes]
A_list = [LambertianPointLight(
    f= np.vstack(f_list[mesh]),
    v=vch[mesh],
    vn=vnch[mesh],
    num_verts=len(vch[mesh]),
    light_pos=light_pos,
    vc=vcch[mesh],
...
vc_list = [A_list[mesh]*vcch[mesh] + vcch[mesh]*light_intensity for mesh in rangeMeshes]


# A_mod = [SphericalHarmonics(vn=vnchmod[mesh],
#                        components=chComponent,
#                        light_color=light_color)]
vcmod_list = [vc_list[0]]
...
imagegt = rn.r.copy()
imagergb = cv2.cvtColor(numpy.uint8(imagegt*255), cv2.COLOR_RGB2BGR)
cv2.namedWindow('gt', cv2.WINDOW_NORMAL)
# cv2.startWindowThread()
cv2.imshow('gt',imagergb)
cv2.waitKey(0)

image = rnmod.r.copy()
imagergb = cv2.cvtColor(numpy.uint8(image*255), cv2.COLOR_RGB2BGR)
cv2.namedWindow('render', cv2.WINDOW_NORMAL)
cv2.imshow('render',imagergb)
cv2.waitKey(0)

# Changes:

chAzScaled[0] = chAzScaled.r + radians(azchanges[experiment])/azScale
chElScaled[0] = chElScaled.r + radians(elchanges[experiment])/elScale
light_intensity[0] = light_intensity.r + light_changes[experiment]
...
chImage = ch.array(imagegt.copy())
E_raw = rnmod - chImage
SqE_raw = ch.SumOfSquares(rn - chImage)/chImage.size
iterat = 0

# image = E_raw.r.copy()
# imagergb = cv2.cvtColor(numpy.uint8(image*255), cv2.COLOR_RGB2BGR)
# cv2.namedWindow('error', cv2.WINDOW_NORMAL)
# cv2.imshow('error',imagergb)
# cv2.waitKey(0)
...
def cb2(_):
    ...
    image = np.copy(np.array(rnmod.r)).astype(np.float64)
    imagergb = cv2.cvtColor(numpy.uint8(image*255), cv2.COLOR_RGB2BGR)
    cv2.imshow('render',imagergb)
...
chDistScaled
chComponentScaled
free_variables = [chAzScaled, chElScaled, light_intensity]
