...
def read_obj(filename):
    ...
    for line in lines:
        ...
        if key == 'v':
            ...
            fname = obj_directory + valuesstring
            print("fname: " + fname)
            if not exists(fname):
                print("NOT EXISTS fname: " + fname)
                fname = valuesstring
            if not exists(fname):
                raise Exception("Can't find path %s" % (valuesstring))
            _update_mtl(mtls, fname)
        elif key == 'usemtl':
            cur_mtl = mtls[valuesstring]
