...
colorRenderer = ColoredRenderer()
colorRendererBackground =  ColoredRenderer()

renderOptions = ['blender', 'opendr']
renderGroundTruth = renderOptions[0]
...
for teapotIdx, teapotTest in enumerate(experimentTeapots):
    ...
    #Prepare experiment storage vars.
    ...
    #Load differentiable renderer data.
    v,f, vc, vn, uv = sceneimport.unpackObjects(teapot)
    ...
    for selTestNum in expSelTest:
        ...
        colorRenderer.camera = ProjectPoints(v=vch, rt=rotation, t=translation, f= 1.12*ch.array([width,width]), c=ch.array([width,height])/2.0, k=ch.zeros(5))
        colorRenderer.camera.openglMat = np.array(mathutils.Matrix.Rotation(radians(180), 4, 'X'))
        colorRenderer.frustum = {'near': clip_start, 'far': clip_end, 'width': width, 'height': height}
        colorRenderer.set(v=vch, f=fch, bgcolor=ch.ones(3)*0.75, overdraw=True)

        if currentScene != sceneNum:

            if renderGroundTruth == 'blender':
                if currentScene != -1:
                    # Cleanup
            ...
            if renderGroundTruth == 'blender':
                ...
                #Unpack and stack scene objects for OpenDr.
                scene_v = []
                scene_vn = []
                scene_vc = []
                scene_uv = []
                ...
                for model in modelInstances:
                    obj_v,obj_f, obj_vc, obj_vn, obj_uv = sceneimport.unpackObjects(model)
                    scene_v = scene_v + [obj_v]
                    scene_f = scene_f + [obj_f + len(scene_f)]
                    scene_vc = scene_vc + [obj_vc]
                    scene_vn = scene_vn + [obj_vn]
                    scene_uv = scene_uv + [obj_uv]

                scene_v = ch.Ch(np.vstack(scene_v).astype(np.float64))
                scene_f = np.vstack(scene_f).astype(np.float64)
                scene_vc = ch.Ch(np.vstack(scene_vc).astype(np.float64))
                scene_vn = ch.Ch(np.vstack(scene_vn).astype(np.float64))
                scene_uv = ch.Ch(np.vstack(scene_uv).astype(np.float64))
                components = [3.,1.,1.,0.,0.,0.,0.,0.,0.],
